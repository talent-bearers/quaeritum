    ...
    public void acceptResults(@Nullable List<? extends ISearchAlgorithm.Result> results) {
        ...
        if (results == null || results.isEmpty()) {
            ...
            for (ISearchAlgorithm.Result resultItem : results) {
                if (resultItem.getFrequency() > largestFrequency)
                    largestFrequency = resultItem.getFrequency();
                if (resultItem.getFrequency() < smallestFrequency)
                    smallestFrequency = resultItem.getFrequency();
            }

            int itemsPerPage = 8;
            int count = 0;
            for (ISearchAlgorithm.Result resultItem : results) {
                double matchPercentage = Math.round(resultItem.getFrequency() * 100.0);
                if (matchPercentage <= 0) continue;

                Entry resultEntry = resultItem.getEntry();
                ...
                GuiComponent indexButton = book.makeNavigationButton(count, resultEntry, plate -> plate.add(textComponent));
                ...
                // --------- HANDLE EXTRA TEXT COMPONENT --------- //
                {
                    ...
                    if (forTyping.isSpecificResult()) {
                        ...
                        exactResult = simplifiedResult = resultItem.getFrequency() == 1 ?
                                I18n.format("librarianlib.book.results.kwd") :
                                I18n.format("librarianlib.book.results.kwds", (int) resultItem.getFrequency());
                    }
