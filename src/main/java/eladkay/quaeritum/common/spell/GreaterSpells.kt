...
object GreaterSpells {
    ...
    init {
        ...
        SpellParser.registerSpell(arrayOf(METAL, CONNECTION)) { player, trailing, total ->
            ...
            applyBuffToOther(PotionIronskin, player, trailing, total, false)
        }
        ...
        SpellParser.registerSpell(arrayOf(FORM, CONNECTION)) { player, trailing, total ->
            ...
            applyBuffToOther(MobEffects.INVISIBILITY, player, trailing, total, false)
        }
        ...
        SpellParser.registerSpell(arrayOf(FLOW, CONNECTION)) { player, trailing, total ->
            ...
            applyBuffToOther(MobEffects.JUMP_BOOST, player, trailing, total, true)
            applyBuffToOther(MobEffects.SPEED, player, trailing, total, true)
        }
        ...
        SpellParser.registerSpell(arrayOf(SOUL, SPIRIT)) { player, trailing, total ->
            ...
            var i = Math.max(2 + trailing - total, 1)
            player.world.getEntitiesInAABBexcluding(player, player.entityBoundingBox.grow(5.0)) {
                it != null && it.getDistanceSqToEntity(player) < 25.0
            }.forEach {
                if (it is EntityLivingBase && i-- != 0)
                    it.addPotionEffect(PotionEffect(PotionRooted))
            }
        }
